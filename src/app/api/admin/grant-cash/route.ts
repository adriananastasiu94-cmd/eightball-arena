...
import { grantFallbackCash } from "@/lib/fallbackWallet";

const grantCashSchema = z.object({
...
export async function POST(request: NextRequest) {
  ...
  const parsed = grantCashSchema.safeParse(body);
  if (!parsed.success) {
  ...
  try {
    ...
    console.error("admin:grant-cash failed", error);
    try {
      const fallback = await grantFallbackCash(targetEmail, amount);
      return NextResponse.json({
        ok: true,
        grantedBy: actor.email,
        target: {
      ...
      console.error("admin:grant-cash fallback failed", fallbackError);
    }
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      return NextResponse.json(
        { error: "Unable to grant cash right now", code: error.code },
        { status: 500 }
      );
    }
    return NextResponse.json({ error: "Unable to grant cash right now" }, { status: 500 });
  }
}
